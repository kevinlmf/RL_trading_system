...
# === Import CPPO agent and network ===
from learning.strategy.cppo.agent import CPPOAgent
from learning.strategy.cppo.network import ActorCritic
...
for env_name, env in environments.items():
    print(f"\n🚀 Training CPPO in [{env_name}] environment...")

    # Auto detect state & action dim
    actual_state_dim = env.observation_space.shape[0]
    action_dim = env.action_space.shape[0]
    print(f"✅ {env_name} | detected state_dim={actual_state_dim}, action_dim={action_dim}")

    # Initialize CPPO agent
    model = ActorCritic(state_dim=actual_state_dim, action_dim=action_dim).to(DEVICE)
    agent = CPPOAgent(actor_critic=model, batch_size=BATCH_SIZE, device=DEVICE)
    ...
    for ep in range(EPISODES):
        ...
        while not done:
            ...
            # ✅ Check if state_dim matches model input
            if state_tensor.shape[1] != actual_state_dim:
                raise ValueError(
                    f"❌ Shape mismatch: ActorCritic expects {actual_state_dim}, "
                    f"but got {state_tensor.shape[1]} from env.reset()"
                )
            ...
            action = torch.nn.functional.one_hot(torch.tensor(action_idx), num_classes=action_dim).cpu().numpy()

            next_state, reward, done, info = env.step(action)

            reward /= INITIAL_CASH  # Normalize reward
            agent.store_transition((state, action_idx, reward, done, log_prob.item(), 0.0))
    ...
    # Save model
    model_path = f"models/cppo_actor_critic_{env_name.lower()}.pt"
...
print("\n📊 Financial Metrics (Copula vs Baseline):")
...
for env_name, rewards in reward_history.items():
    ...
    plt.plot(smooth_rewards, label=f"{env_name} CPPO")
...
plt.title("CPPO Training: Copula vs Baseline")
plt.xlabel("Episode")
plt.ylabel("Smoothed Total Reward (normalized)")
plt.legend()
plt.grid(True)
plt.tight_layout()
plot_path = "results/cppo_copula_vs_baseline_metrics.png"
